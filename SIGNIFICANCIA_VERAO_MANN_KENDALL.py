...
# --- 4. Caminho dos dados ---
input_folder = '/content/drive/MyDrive/MESTRADO/01. DADOS/02.ANO A ANO MEDIANAS/SEM EXCLUSÃO/VERÃO'
...
def processar_significancia_independente(ano_ini, ano_fim, sufixo_saida):
    ...
    output_path = os.path.join(input_folder, f'Mapa_Significancia_Verao_{sufixo_saida}.tif')
...
if mapa_sig_p1 is not None and mapa_sig_p2 is not None:
    ...
    ax1.set_title(f"Significância Estatística Verão (2000-2013)\n(p < 0.05)")

    ax2.imshow(mapa_sig_p2, cmap='RdYlGn_r')
    ax2.set_title(f"Significância Estatística Verão (2014-2024)\n(p < 0.05)")
