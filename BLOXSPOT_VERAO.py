...
folder_path = r'/content/drive/MyDrive/MESTRADO/01. DADOS/02.ANO A ANO MEDIANAS/SEM EXCLUSÃO/VERÃO'
...
def extrair_dados_por_periodo(ano_inicio, ano_fim):
    dados_acumulados = []
    arquivos = [f for f in os.listdir(folder_path) if f.startswith('LST_20') and f.endswith('.tif')]
...
if not df_base.empty and not df_pos.empty:
    ...
    ax1.set_title('Variabilidade Térmica (TSC) Verão: 2000-2013', fontweight='bold')
    ax1.set_ylabel('Temperatura (°C)')
    ax1.grid(axis='y', alpha=0.3)

    sns.boxplot(x='AP', y='LST', data=df_pos, ax=ax2, order=ordem_aps, color='salmon', showfliers=True)
    ax2.set_title('Variabilidade Térmica (TSC) Verão: 2014-2024', fontweight='bold')
